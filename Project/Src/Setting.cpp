...
/*static*/ BuildingSetting Setting::GetInstance(const Vector2 & position)
{
	...
	for (uint64_t nPoint = 0; nPoint < InfluencePointsCount; ++nPoint)
	{
		...
		instance.Size.Min += InfluencePoints[nPoint].setting.Size.Min * y + InfluencePoints[nPoint].border.Size.Min * z;
		instance.Size.Max += InfluencePoints[nPoint].setting.Size.Max * y + InfluencePoints[nPoint].border.Size.Max * z;
