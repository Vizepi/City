#include <iostream>

#include <Triangle.h>
#include <Quad.h>
#include <Random.h>
#include <Object.h>

void Lyon();

int main(int argc, char **argv)
{
	Setting::InfluencePointsCount = 5;
	
	Setting::InfluencePoints[0] = {
		{ { 20.0, 35.0 }, { 5.0, 25.0 }, { 100.0, 200.0 }, 100.0, 0.01, 5.1, 0.1 },
		{ { 15.0, 20.0 }, { 5.0, 15.0 }, { 40.0, 120.0 }, 50.0, 0.001, 4.8, 0.05 },
		Vector2(-3400.0, -3000.0),
		1600.0 };
	Setting::InfluencePoints[1] = {
		{ { 25.0, 50.0 }, { 15.0, 40.0 }, { 140.0, 300.0 }, 150.0, 0.02, 5.1, 0.1 },
		{ { 15.0, 20.0 }, { 5.0, 20.0 }, { 60.0, 160.0 }, 50.0, 0.001, 4.8, 0.05 },
		Vector2(2800.0, 3200.0),
		1900.0 };
	Setting::InfluencePoints[2] = {
		{ { 40.0, 55.0 }, { 30.0, 50.0 }, { 160.0, 400.0 }, 200.0, 0.05, 5.1, 0.1 },
		{ { 15.0, 25.0 }, { 15.0, 20.0 }, { 80.0, 200.0 }, 55.0, 0.005, 4.8, 0.05 },
		Vector2(300.0, -800.0),
		3200.0 };
	Setting::InfluencePoints[3] = {
		{ { 25.0, 60.0 }, { 15.0, 35.0 }, { 100.0, 400.0 }, 100.0, 0.03, 5.1, 0.1 },
		{ { 10.0, 25.0 }, { 10.0, 20.0 }, { 80.0, 160.0 }, 40.0, 0.002, 4.8, 0.05 },
		Vector2(-800.0, 3000.0),
		1100.0 };
	Setting::InfluencePoints[4] = {
		{ { 20.0, 45.0 }, { 20.0, 35.0 }, { 100.0, 175.0 }, 100.0, 0.01, 5.1, 0.1 },
		{ { 15.0, 25.0 }, { 10.0, 20.0 }, { 60.0, 100.0 }, 30.0, 0.005, 4.8, 0.05 },
		Vector2(2600.0, 2800.0),
		2200.0 };
	/*Setting::InfluencePoints[0] = {
		{ { 20.0, 35.0 }, { 10.0, 30.0 }, { 100.0, 200.0 }, 100.0, 0.01, 5.1, 0.2 },
		{ { 15.0, 20.0 }, { 10.0, 20.0 }, { 40.0, 120.0 }, 50.0, 0.001, 4.8, 0.1 },
		Vector2(-3400.0, -3000.0),
		1600.0 };
	Setting::InfluencePoints[1] = {
		{ { 25.0, 50.0 }, { 20.0, 50.0 }, { 140.0, 300.0 }, 250.0, 0.02, 5.1, 0.2 },
		{ { 15.0, 20.0 }, { 10.0, 30.0 }, { 60.0, 160.0 }, 60.0, 0.001, 4.8, 0.1 },
		Vector2(2800.0, 3200.0),
		1900.0 };
	Setting::InfluencePoints[2] = {
		{ { 40.0, 55.0 }, { 50.0, 80.0 }, { 160.0, 400.0 }, 300.0, 0.05, 5.1, 0.2 },
		{ { 15.0, 25.0 }, { 15.0, 30.0 }, { 80.0, 200.0 }, 75.0, 0.005, 4.8, 0.1 },
		Vector2(300.0, -800.0),
		3200.0 };
	Setting::InfluencePoints[3] = {
		{ { 25.0, 60.0 }, { 20.0, 45.0 }, { 100.0, 400.0 }, 150.0, 0.03, 5.1, 0.2 },
		{ { 10.0, 25.0 }, { 15.0, 30.0 }, { 80.0, 160.0 }, 50.0, 0.002, 4.8, 0.1 },
		Vector2(-800.0, 3000.0),
		1100.0 };
	Setting::InfluencePoints[4] = {
		{ { 20.0, 45.0 }, { 25.0, 55.0 }, { 100.0, 175.0 }, 150.0, 0.01, 5.1, 0.2 },
		{ { 15.0, 25.0 }, { 15.0, 30.0 }, { 60.0, 100.0 }, 30.0, 0.005, 4.8, 0.1 },
		Vector2(2600.0, 2800.0),
		2200.0 };*/
	/*Setting::MaxNeighborhoodSize = 400.0;

	Random::Seed(0);
	Quad q(Random::NextUInt64(), Vector2(-5000, 5000), Vector2(-5000, -5000), Vector2(5000, -5000), Vector2(5000, 5000));

    Object obj("Output/test.obj");
	q.Subdivide(obj);

	std::cout << "Complete" << std::endl;
	getchar();*/
	Lyon();
	return 0;
}
